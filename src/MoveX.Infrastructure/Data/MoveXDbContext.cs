...
public class MoveXDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<CustomerProfile>(e =>
        {
            ...
            e.HasIndex(x => x.UserId).IsUnique();
            e.HasOne(x => x.Addresses).WithOne();
        });
        ...
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.Description).HasMaxLength(250).IsRequired();
            e.Property(x => x.WeightKg).HasPrecision(12, 2);
            e.Property(x => x.LengthCm).HasPrecision(12, 2);
            e.Property(x => x.WidthCm).HasPrecision(12, 2);
            e.Property(x => x.HeightCm).HasPrecision(12, 2);
        ...
        modelBuilder.Entity<DispatchJob>(e =>
        {
            ...
            e.HasIndex(x => x.AssignedDriverId);
        });

        modelBuilder.Entity<DriverAssignment>(e =>
        {
            e.HasKey(x => x.Id);
        ...
        {
            e.HasKey(x => x.Id);
            e.HasIndex(x => new { x.DriverId, x.RecordedAt });
        ...
        {
            e.HasKey(x => x.Id);
            e.HasIndex(x => new { x.Status, x.Severity, x.CreatedAt });
            e.HasIndex(x => x.BookingId);
            e.HasIndex(x => x.DriverId);
        });

        modelBuilder.Entity<MovingService>(e =>
        {
            e.HasKey(x => x.Id);
        ...
        modelBuilder.Entity<PricingRule>(e =>
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.Name).HasMaxLength(150).IsRequired();
            e.Property(x => x.BaseFare).HasPrecision(18, 2);
            e.Property(x => x.PerKmRate).HasPrecision(18, 2);
            e.Property(x => x.PerMinuteRate).HasPrecision(18, 2);
            e.Property(x => x.MinimumFare).HasPrecision(18, 2);
            e.Property(x => x.LoadingFee).HasPrecision(18, 2);
            e.Property(x => x.UnloadingFee).HasPrecision(18, 2);
            e.Property(x => x.NightSurchargePercent).HasPrecision(8, 4);
        ...
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.ActualDistanceKm).HasPrecision(12, 2);
            e.Property(x => x.ActualDurationMinutes).HasPrecision(12, 2);
        ...
        modelBuilder.Entity<TripWaypoint>(e =>
        {
            e.HasKey(x => x.Id);
            e.HasIndex(x => new { x.TripId, x.Sequence }).IsUnique();
            e.Property(x => x.Latitude).HasPrecision(10, 7);
            e.Property(x => x.Longitude).HasPrecision(10, 7);
            e.HasOne(x => x.Trip).WithMany(x => x.Waypoints).HasForeignKey(x => x.TripId).OnDelete(DeleteBehavior.Cascade);
        });

        modelBuilder.Entity<DeliveryProof>(e =>
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.FileUrl).HasMaxLength(2048);
            e.HasIndex(x => new { x.TripId, x.Type });
            e.HasOne(x => x.Trip).WithMany(x => x.DeliveryProofs).HasForeignKey(x => x.TripId).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        modelBuilder.Entity<DriverPayout>(e =>
        {
            e.HasKey(x => x.Id);
            ...
            e.HasIndex(x => new { x.DriverId, x.PeriodStart, x.PeriodEnd });
            e.HasOne(x => x.Driver).WithMany().HasForeignKey(x => x.DriverId).OnDelete(DeleteBehavior.Restrict);
        });
    }
}
